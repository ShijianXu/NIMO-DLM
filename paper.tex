...
\title{%
    \textbf{Compositional Feature Interpretability for Diffusion Language Models}%
}

\author{
    Shijian Xu \\
    University of Basel \\
    \texttt{shijian.xxu@gmail.com}
}
...
We apply NIMO---a nonlinear interaction model with a closed-form linear decomposition---to sparse autoencoder (SAE) features extracted from LLaDA-8B-Base across six transformer layers and five denoising timesteps.
...
In contrast, a causal autoregressive baseline (GPT-2 Small with Joseph Bloom SAEs) shows $\LR \ge 0.30$ across all layers and prefix lengths, confirming that bidirectional attention generates qualitatively stronger feature interactions than unidirectional attention.
...
Diffusion language models (dLLMs) present a particularly interesting setting for this question.
...
LLaDA-8B-Base~\citep{llada2024} is an 8B-parameter masked diffusion language model based on a bidirectional transformer (32~layers, $d_\mathrm{model} = 4{,}096$).
...
At noise level $t$, the model conditions on all visible tokens simultaneously, creating dense bidirectional attention patterns that differ fundamentally from autoregressive models.
...
\end{equation}
retaining only the $K = 50$ largest activations. The resulting feature vectors are ultra-sparse: only 50 out of 16,384 dimensions are nonzero per sample (sparsity ratio 99.7\%).
...
We sample 30,000 sequences from the Pile validation split (512 tokens each) and run LLaDA-8B-Base forward at noise levels $t \in \{0.10, 0.25, 0.50, 0.75, 1.00\}$.
...
For comparison, we extract features from GPT-2 Small (12 layers, $d_\mathrm{model} = 768$) using Joseph Bloom's SAEs~\citep{bloom2024sae} ($d_\mathrm{SAE} = 24{,}576$) at layers $\{0, 2, 4, 6, 8, 10\}$.
...
\subsection{RQ2: Phase Transition in LR($t$)}
...
\toprule
\textbf{Layer} & \textbf{Depth} & $t=0.10$ & $t=0.25$ & $t=0.50$ & $t=0.75$ & $\Delta(0.10 \to 0.75)$ \\
\midrule
...
\subsection{RQ1: Feature Beta Stability Across Timesteps}
...
\toprule
\textbf{Layer} & \textbf{Always-on} & \textbf{Early-dominant} & \textbf{Late-dominant} & \textbf{Total active} \\
\midrule
...
\subsection{RQ3: Feature Rankings and Causal Relevance}
...
The top-10 $|\beta|$ features shift substantially across timesteps within a layer. In layer~26, the leading feature at $t = 0.10$ (SAE index~6185, $|\beta| = 0.053$) is replaced by a different feature (index~861, $|\beta| = 0.071$) at $t = 0.75$, indicating that the most linearly predictive feature changes as context availability varies. Full rankings are saved in \texttt{results/causal\_ranking.pt} for downstream activation-patching experiments.

\subsection{RQ4: Feature Interaction Sparsity}
...
This extreme sparsity is consistent with the mechanistic interpretability literature suggesting that SAE features interact through small circuits rather than all-to-all dependencies.

\subsection{RQ5: dLLM vs.\ AR-LLM Compositionality}

\begin{table}[h]
\centering
\caption{Linearity Ratio $\LR$ for GPT-2 Small (AR) across prefix fractions. ``Flat?'' indicates whether the standard deviation across prefix fractions is $< 0.05$.}
\label{tab:lr_ar}
\begin{tabular}{rr ccccc rr}
\toprule
\textbf{Layer} & \textbf{Depth} & $t=0.10$ & $t=0.25$ & $t=0.50$ & $t=0.75$ & $t=1.00$ & \textbf{Std} & \textbf{Flat?} \\
\midrule
...
\toprule
\textbf{Layer} & $t$ & \textbf{Linear $R^2$} & \textbf{NIMO $R^2$} & \textbf{Improvement} \\
\midrule
...
\begin{itemize}
    \item \textbf{Single dLLM architecture.} Results are specific to LLaDA-8B-Base with DLM-Scope SAEs. Whether other dLLM architectures (e.g., MDLM, Plaid) show the same phase transition remains an open question.
    ...
    \item \textbf{Causal steering not yet executed.} The feature rankings in \texttt{results/causal\_ranking.pt} enable activation patching experiments (comparing $|\beta|$-ranked vs.\ frequency-ranked vs.\ correlation-ranked features), but these experiments are not yet implemented.
\end{itemize}
...
\begin{tabular}{rr ccccc r}
\toprule
\textbf{Layer} & \textbf{Depth} & $t=0.10$ & $t=0.25$ & $t=0.50$ & $t=0.75$ & $t=1.00^*$ & \textbf{NIMO $R^2$ ($t{=}0.10$)} \\
\midrule
1  & 3.1\%  & 0.790 & 0.814 & 0.855 & 0.878 & 0.523 & 0.049 \\
6  & 18.8\% & 0.624 & 0.659 & 0.719 & 0.775 & 0.969 & 0.070 \\
11 & 34.4\% & 0.527 & 0.559 & 0.623 & 0.680 & 0.996 & 0.155 \\
16 & 50.0\% & 0.434 & 0.462 & 0.531 & 0.584 & 0.952 & 0.179 \\
26 & 81.3\% & 0.199 & 0.224 & 0.259 & 0.303 & 0.081 & 0.304 \\
30 & 93.8\% & 0.094 & 0.108 & 0.124 & 0.135 & 0.414 & 0.303 \\
\bottomrule
\multicolumn{8}{l}{$^*$Degenerate: only 50 mask-token features active; LR unreliable.}
\end{tabular}
\end{table}
...
\toprule
\textbf{Layer} & \textbf{Depth} & $t=0.10$ & $t=0.25$ & $t=0.50$ & $t=0.75$ & $t=1.00$ \\
\midrule
0  & 0\%   & 0.975 & 0.975 & 0.973 & 0.971 & 0.970 \\
2  & 17\%  & 0.610 & 0.591 & 0.588 & 0.598 & 0.561 \\
4  & 33\%  & 0.564 & 0.523 & 0.522 & 0.532 & 0.538 \\
6  & 50\%  & 0.538 & 0.484 & 0.468 & 0.480 & 0.499 \\
8  & 67\%  & 0.505 & 0.439 & 0.363 & 0.377 & 0.389 \\
10 & 83\%  & 0.457 & 0.371 & 0.301 & 0.303 & 0.302 \\
\bottomrule
\end{tabular}
\end{table}
